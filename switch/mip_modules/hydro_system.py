...
def define_components(mod):
    ...
    initial_res_vol[r, ts] is a parameter that states the starting volume of
    ...
    final_res_vol[r, ts] is a parameter that states the final volume of stored
    ...
    level at the end of each timeseries.
    ...
    # TODO: maybe make the validation rules below enforce against min and max
    # volume for first or last timepoint of series, rather than overall
    # reservoir limits
    mod.initial_res_vol = Param(
        ...
        or (m.res_min_vol_tp[r] <= val <= m.res_max_vol[r]),
    )
    mod.final_res_vol = Param(
        ...
        default=float("inf"),
        validate=lambda m, val, r, ts: val == float("inf")
        or (m.res_min_vol[r] <= val <= m.res_max_vol[r]),
    )
    ...
    mod.Set_initial_res_vol_If_Setting_final_res_vol = BuildCheck(
        mod.TIMESERIES,
        rule=lambda m, ts: m.final_res_vol == float("inf")
        or m.initial_res_vol != float("inf"),
    )
    ...
    # apply initial_res_vol if provided
    mod.Enforce_Initial_Res_Vol = Constraint(
        mod.RESERVOIRS,
        mod.TIMESERIES,
        rule=lambda m, r, ts: (
            Constraint.Skip
            if (m.initial_res_vol[r, ts] == float("inf"))  # not specified
            else (
                m.ReservoirVol[r, m.TPS_IN_TS[ts].first()] == m.initial_res_vol[r, ts]
            )
        ),
    )
    ...
    def Enforce_Wnode_Balance_rule(m, wn, tp):
        ...
        if wn in m.RESERVOIRS:
            ts = m.tp_ts[tp]
            if tp == m.TPS_IN_TS[ts].last() and m.final_res_vol[wn, ts] != float("inf"):
                # reach final_res_vol if specified
                end_of_tp_volume = m.final_res_vol[wn, ts]
            else:
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        optional=True,
        param=(mod.initial_res_vol, mod.final_res_vol),
    )
    switch_data.load_aug(

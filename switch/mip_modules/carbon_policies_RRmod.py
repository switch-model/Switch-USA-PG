...
def define_components(model):
    ...
    model.AnnualCapViolation = Var(
        model.PERIODS,
        within=NonNegativeReals,
    )

    def Carbon_Cap_rule(m, p):
        if m.carbon_cap_tco2_per_yr[p] != float(
            "inf"
        ) and m.carbon_cost_dollar_per_tco2[p] == float("inf"):
            return m.AnnualEmissions[p] <= m.carbon_cap_tco2_per_yr[p]
        elif m.carbon_cap_tco2_per_yr[p] != float("inf"):
            return (
                m.AnnualEmissions[p]
                <= m.carbon_cap_tco2_per_yr[p] + m.AnnualCapViolation[p]
            )
        else:
    ...
    model.EmissionsCosts = Expression(
        model.PERIODS,
        rule=lambda model, period: model.AnnualCapViolation[period]
        * model.carbon_cost_dollar_per_tco2[period],
        # if model.AnnualEmissions[period] >= model.carbon_cap_tco2_per_yr[period]
        # else model.AnnualEmissions[period] * model.carbon_cost_dollar_per_tco2[period],
